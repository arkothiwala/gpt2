...
if __name__ == '__main__':
    ...
    logger.info(f"Starting training loop from global step {global_step}, batch index {batch_idx_start}, global batch size {global_batch_size}, micro batch size {exp_config.get('training').get('micro_batch_size')}")
    ...
    #         next(valid_iter)
    for batch_idx, (batch_x_train, batch_y_train) in enumerate(tqdm(valid_iter, desc="epoch's batch progress"), start=batch_idx_start):
        ...
        if total_accumulated % global_batch_size == 0:
            ...
            logger.info(f"Step {global_step} | global_batch_loss = {global_batch_loss} | perplexity = {perplexity} | unclipped_grad_norm = {unclipped_grad_norm} | clipped_grad_norm = {clipped_grad_norm}")
